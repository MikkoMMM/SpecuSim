...
#        self.old_camera_z = self.camera.getZ(self.render)
            self.node_pos = LVector3f( rY*math.cos(self.ang)*radius, -rY*math.sin(self.ang)*radius, radius*math.cos( self.angY) )
        else: # Could also be used when reparented to render but there is a focus point
            self.node_pos = self.focus_point + LVector3f( rY*math.cos(self.ang)*radius, -rY*math.sin(self.ang)*radius, radius*math.cos( self.angY) )
